...
namespace UnityScreenNavigator.Runtime.Core.Modal
{
    ...
    public class Popup : MonoBehaviour, IPopupLifecycleEvent
    {
        ...
        public void RemoveLifecycleEvent(IPopupLifecycleEvent lifecycleEvent)
        {
        ...
        internal AsyncProcessHandle BeforeEnter(bool push, Popup partnerPopup)
        {
            return CoroutineManager.Instance.Run(BeforeEnterRoutine(push, partnerPopup));
        }
        ...
        {
            return CoroutineManager.Instance.Run(EnterRoutine(push, playAnimation, partnerPopup));
        }
        ...
        private IEnumerator EnterRoutine(bool push, bool playAnimation, Popup partnerPopup)
        {
            if (push)
            {
                ...
                if (playAnimation)
                {
                    var anim = _animationContainer.GetAnimation(true, partnerPopup?.Identifier);
                    if (anim == null)
        ...
        internal AsyncProcessHandle BeforeExit(bool push, Popup partnerPopup)
        {
            return CoroutineManager.Instance.Run(BeforeExitRoutine(push, partnerPopup));
        }
        ...
        internal AsyncProcessHandle Exit(bool push, bool playAnimation, Popup partnerPopup)
        {
            return CoroutineManager.Instance.Run(ExitRoutine(push, playAnimation, partnerPopup));
        }
        ...
        private IEnumerator ExitRoutine(bool push, bool playAnimation, Popup partnerPopup)
        {
            if (!push)
            {
                if (playAnimation)
                {
                    var anim = _animationContainer.GetAnimation(false, partnerPopup?._identifier);
                    if (anim == null)
