...
namespace UnityScreenNavigator.Runtime.Core.Modal
{
    ...
    public class PopupContainer : MonoBehaviour, IScreenContainer
    {
            ...
            PopupFactory pageFactory)
        {
            ...
            Debug.Log("Resolve PopupContainer");
        }
        ...
        /// </summary>
        public IReadOnlyDictionary<string, Popup> Modals => _modals;
        ...
        /// <summary>
        ///     Get the <see cref="PopupContainer" /> that manages the modal to which <see cref="transform" /> belongs.
        /// </summary>
        /// <param name="transform"></param>
        /// <param name="useCache">Use the previous result for the <see cref="transform" />.</param>
        /// <returns></returns>
        public static PopupContainer Of(Transform transform, bool useCache = true)
        {
            return Of((RectTransform)transform, useCache);
        }

        /// <summary>
        ///     Get the <see cref="PopupContainer" /> that manages the modal to which <see cref="rectTransform" /> belongs.
        /// </summary>
        ...
        public AsyncProcessHandle Push(
            ...
            string modalId = null,
            bool loadAsync = true,
            Action<(string modalId, Popup modal)> onLoad = null
        )
        {
            return CoroutineManager.Instance.Run(PushRoutine(typeof(Popup),
                resourceKey,
                playAnimation,
                onLoad,
                loadAsync,
        ...
        )
        {
            ...
            Popup popup = null;
            yield return LoadModalAndBind(modalType,
                resourceKey,
                loadAsync,
                (m, lh) =>
                {
            ...
            var context = ModalPushContext.Create(modalId, popup, _orderedModalIds, _modals);
            ...
            _modals.Add(context.ModalId, context.EnterPopup);
        ...
        private IEnumerator LoadModal(
            ...
            string resourceKey,
            bool loadAsync,
            Action<Popup, AssetLoadHandle<GameObject>> onLoaded
        )
        {
            var assetLoadHandle = loadAsync
                ? AssetLoader.LoadAsync<GameObject>(resourceKey)
        ...
        private IEnumerator LoadModalAndBind(
            Type modalType,
            string resourceKey,
            bool loadAsync,
            Action<Popup, AssetLoadHandle<GameObject>> onLoaded
        )
        {
            ...
            if (assetLoadHandle.Status == AssetLoadStatus.Failed)
                throw assetLoadHandle.OperationException;
            var page = _prefabFactory.Create<Popup>(assetLoadHandle.Result);// Instantiate(assetLoadHandle.Result);
